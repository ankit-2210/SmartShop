...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const allSubOptions = Array.from(subcategorySelect.querySelectorAll("option")
                                   .filter(opt => opt.value !== ""));

    function filterSubcategories() {
        const selectedCategory = categorySelect.value;

        // Save current selected subcategory value
        ...
        allSubOptions
            .filter(opt => opt.dataset.category === selectedCategory)
            .forEach(opt => subcategorySelect.appendChild(opt.cloneNode(true)));

        // Restore selected subcategory if still valid
        const restoreOption = Array.from(subcategorySelect.options)
            .find(opt => opt.value === currentSubValue);

        if (restoreOption) {
            restoreOption.selected = true;
        }
        else {
            subcategorySelect.selectedIndex = 0; // placeholder
        }
    }
    ...
    // Trigger filter on page load (edit mode support)
...
console.log("Click");
//    const cid = parseInt(button.getAttribute("data-id"));
...
//          window.location="/user/cartQuantityUpdate?sy=de&cid="+cid;
        }
      });
    });
    ...
    increaseButtons.forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        if (currentValue < parseInt(quantityInput.getAttribute('max'))) {
          quantityInput.value = currentValue + 1;
//          window.location="/user/cartQuantityUpdate?sy=in&cid="+cid;
        }
      });
    });
  ...
  function priceRangeWidget() {
    ...
    priceRangeWidgets.forEach(widget => {
      ...
      function updateSliderProgress() {
        ...
        const maxPercent = ((maxValue - sliderM"order-progressin) / range) * 100;
